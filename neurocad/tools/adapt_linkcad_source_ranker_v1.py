"""Adapt the frozen LinkCAD part-language ranker to a candidate source."""
...
from neurocad.linkcad_source_adaptation_v1 import (
    SCHEMA_VERSION,
    configure_part_ranker_adaptation_v1,
    part_assignment_loss_v1,
...
def main() -> None:
  ...
  parser.add_argument("--adapt-part-encoder", action="store_true")
  parser.add_argument("--device", default="cuda" if torch.cuda.is_available() else "cpu")
  args = parser.parse_args()
  if args.epochs < 1 or args.learning_rate <= 0:
    raise ValueError("LinkCAD source-adaptation training configuration differs")
  ...
  parameters = configure_part_ranker_adaptation_v1(
      model, adapt_part_encoder=args.adapt_part_encoder,
  )
  ...
  for epoch in range(args.epochs):
    ...
    for step, index in enumerate(order, start=1):
      ...
      loss = part_assignment_loss_v1(model, query)
      (loss / 8.0).backward()
  ...
  checkpoint = args.output_dir / f"adapted_seed{args.seed}.pt"
  torch.save(model.state_dict(), checkpoint)
  payload = {
      "schema_version": SCHEMA_VERSION,
      "scope": "development_generated_source_part_ranker_adaptation",
      ...
      "seed": args.seed,
      "adapt_part_encoder": args.adapt_part_encoder,
